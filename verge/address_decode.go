...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := verge_addrdec.XVG_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = verge_addrdec.XVG_testnetPrivateWIFCompressed
	}

	wif, _ := verge_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	verge_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := verge_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := verge_addrdec.XVG_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = verge_addrdec.XVG_testnetAddressP2SH
	}
	...
	address, _ := verge_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := verge_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
...
func (decoder *addressDecoder) ScriptPubKeyToBech32Address(scriptPubKey []byte) (string, error) {
	...
	cfg := verge_addrdec.XVG_mainnetAddressBech32V0
	if decoder.wm.Config.IsTestNet {
		cfg = verge_addrdec.XVG_testnetAddressBech32V0
	}
