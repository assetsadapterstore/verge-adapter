...
import (
	...
	"github.com/blocktree/go-owcdrivers/vergeTransaction"
...
func (decoder *TransactionDecoder) SignBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			//签名交易
			/////////交易单哈希签名
			signature, err := vergeTransaction.SignTransaction(txHash, keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		...
		sigPub     = make([]vergeTransaction.SigPub, 0)
	...
	for accountID, keySignatures := range rawTx.Signatures {
		...
		for _, keySignature := range keySignatures {
			...
			signaturePubkey := vergeTransaction.SigPub{
				Signature: signature,
				Pubkey:    pubkey,
			}
	...
	pass, signedTrans, err := vergeTransaction.VerifyAndCombineTransaction(emptyTrans, sigPub)
	if pass {
...
func (decoder *TransactionDecoder) createBTCRawTransaction(
	...
	var (
		err              error
		vins             = make([]vergeTransaction.Vin, 0)
		vouts            = make([]vergeTransaction.Vout, 0)
		totalSend        = decimal.New(0, 0)
		destinations     = make([]string, 0)
		accountTotalSent = decimal.Zero
	...
	for _, utxo := range usedUTXO {
		in := vergeTransaction.Vin{utxo.TxID, uint32(utxo.Vout), utxo.ScriptPubKey}
		vins = append(vins, in)
	...
	for to, amount := range to {
		...
		out := vergeTransaction.Vout{to, intAmount}
		vouts = append(vouts, out)
	}

	lockTime := uint32(0)
	//timeInterval := uint32(21600)
	//timeInterval := uint32(21526)
	timeInterval := uint32(0)

	/////////构建空交易单
	emptyTrans, transHash, err := vergeTransaction.CreateEmptyTransactionAndHash(vins, vouts, lockTime, timeInterval)
	if err != nil {
